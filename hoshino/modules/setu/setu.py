...
@sv.on_rex(re.compile(r'^不够[涩瑟色]|^[涩瑟色]图|来一?[点份张][涩瑟色]图|再来[点份张]'))
async def random_setu(bot: NoneBot, ctx):
    keyword = ""
    flag = False
...
@sv.on_rex(re.compile(r'^来[点份张][^劲爆]+[涩瑟色]图'))
async def keyword_setu(bot: NoneBot, ctx):
...
@sv.on_rex(re.compile(r'^劲爆[涩瑟色]图|来一?[点份张]劲爆[涩瑟色]图'))
async def r18_random_setu(bot: NoneBot, ctx):
...
@sv.on_rex(re.compile(r'^来[点份张]劲爆.+[涩瑟色]图'))
async def r18_keyword_setu(bot: NoneBot, ctx):
    if not _r18lmt.check(ctx['user_id']):
        await bot.send(ctx, '您今天已经冲过够劲的了', at_sender=True)
        return
...
@sv.on_rex(r'setu充值')
async def kakin(bot: NoneBot, ctx):
    if ctx['user_id'] not in bot.config.SUPERUSERS:
        return
    count = 0
...
async def online_setu(ctx, keyword, flag, bot):
    ...
    params = {
        "apikey": "",  # 你的apikey
        "size1200": ""  # 是否限制大小，小水管之友,阔佬可以改成False
    }
    if flag:
        params["r18"] = "1"
    if keyword != "":
